 ...
 * @return array
 */
function jackdaniels_body_classes( $classes ) {
...
add_filter( 'body_class', 'jackdaniels_body_classes' );
 ...
 * @return array
 */
function jackdaniels_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'jackdaniels_post_classes', 10, 3 );
 ...
 * @return void
 */
function jackdaniels_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'jackdaniels_pingback_header' );
 ...
 * @return void
 */
function jackdaniels_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'jackdaniels_supports_js' );
 ...
 * @return array
 */
function jackdaniels_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'jackdaniels_comment_form_defaults' );
 ...
 * @return bool
 */
function jackdaniels_can_show_post_thumbnail() {
	/**
	...
	return apply_filters(
		'jackdaniels_can_show_post_thumbnail',
 ...
 * @return int
 */
function jackdaniels_get_avatar_size() {
	return 60;
}
 ...
 * @since jackdaniels 1.0
 */
function jackdaniels_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Post title. Only visible to screen readers. */
		esc_html__( 'Continue reading %s', 'jackdaniels' ),
 ...
 * @since jackdaniels 1.0
 */
function jackdaniels_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . jackdaniels_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'jackdaniels_continue_reading_link_excerpt' );
 ...
 * @since jackdaniels 1.0
 */
function jackdaniels_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . jackdaniels_continue_reading_text() . '</a></div>';
	}
}

// Filter the content more link.
add_filter( 'the_content_more_link', 'jackdaniels_continue_reading_link' );

if ( ! function_exists( 'jackdaniels_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function jackdaniels_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'jackdaniels' ) : $title;
	}
}
add_filter( 'the_title', 'jackdaniels_post_title' );
 ...
 * @return string
 */
function jackdaniels_get_icon_svg( $group, $icon, $size = 24 ) {
	return jackdaniels_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function jackdaniels_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? jackdaniels_get_icon_svg( 'ui', 'arrow_right' ) : jackdaniels_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? jackdaniels_get_icon_svg( 'ui', 'arrow_left' ) : jackdaniels_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'jackdaniels_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function jackdaniels_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'jackdaniels_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'jackdaniels_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'jackdaniels_generate_css' ) ) {
	...
	return jackdaniels_generate_css( // @phpstan-ignore-line.
 ...
 */
function jackdaniels_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 *
 * @since jackdaniels 1.0
 * @since jackdaniels 1.4 Corrected parameter name for `$output`,
 ...
 */
function jackdaniels_password_form( $output, $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'jackdaniels' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'jackdaniels' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'jackdaniels' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'jackdaniels' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'jackdaniels_password_form', 10, 2 );
 ...
 */
function jackdaniels_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'jackdaniels_get_attachment_image_attributes', 10, 3 );

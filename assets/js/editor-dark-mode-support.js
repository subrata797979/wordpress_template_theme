/* global jackdanielsIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'jackdaniels-supports-dark-theme' ) ) {
	jackdanielsDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function jackdanielsDarkModeEditorInit( attempt ) {
	...
	if ( jackdanielsIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						jackdanielsDarkModeEditorInit( attempt + 1 );
					},
